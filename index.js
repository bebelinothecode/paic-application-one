...
let insertquery = `INSERT INTO call_detail_records(
    ...
    IMSI, STATUS, TYPE, TSTAMP, LOCAL_DIALOG_ID, REMOTE_DIALOG_ID, DIALOG_DURATION, USSD_STRING, ID) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`;
...
// connect2db
 ...
 fs.createReadStream(filepath)
    ...
    .on("data", function(row) {
        ...
        connection.query(insertquery, [row[0],row[1],row[2],row[3],row[4],row[5],NULL,row[7],row[8],row[9],
        row[10],row[11],row[12],row[13],row[14],row[15],row[16],row[17], row[18], row[19],row[20],NULL,NULL,
